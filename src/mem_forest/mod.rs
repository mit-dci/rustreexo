...
impl<Hash: AccumulatorHash> Node<Hash> {
    ...
    pub fn read_one<R: Read>(reader: &mut R) -> io::Result<(Rc<Self>, HashMap<Hash, Weak<Self>>)> {
        fn read_one_inner<Hash: AccumulatorHash, R: Read>(
            ...
            if !data.is_empty() {
                let left = read_one_inner(Some(node.clone()), reader, index, depth + 1)?;
                let right = read_one_inner(Some(node.clone()), reader, index, depth + 1)?;
        ...
        let root = read_one_inner(None, reader, &mut index, 0)?;
        Ok((root, index))
    }
