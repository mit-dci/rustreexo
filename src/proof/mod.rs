...
impl<Hash: AccumulatorHash> Proof<Hash> {
    /// Creates a proof from a vector of target and hashes, using a different hash type.
    /// `targets` are u64s and indicates the position of the leaves we are
    /// trying to prove.
    /// `hashes` are of type `AccumulatorHash` and are all hashes we need for computing the roots.
    ///
    ...
    ///
    /// It will compute all roots that contain elements in the proof, by hashing the nodes
    ...
    pub(crate) fn calculate_hashes_delete(
        ...
        num_leaves: u64,
    ) -> Result<RootsOldNew<Hash>, ProofError> {
    ...
    /// This differs from `calculate_hashes_delete` as this one is only used for verifying
    ...
    /// doesn't return nodes that were calculated, as we won't need them for updating the
    /// accumulator.
